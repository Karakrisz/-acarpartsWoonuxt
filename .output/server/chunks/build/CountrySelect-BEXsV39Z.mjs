import { useSSRContext, watch, unref, mergeProps } from 'vue';
import { ssrRenderAttrs, ssrIncludeBooleanAttr, ssrRenderList, ssrRenderAttr, ssrInterpolate } from 'vue/server-renderer';
import { i as useState, r as GqlGetAllowedCountries, s as GqlGetStates } from './server.mjs';
...
const useCountry = () => {
  const allowedCountries = useState("allowedCountries", () => null);
  const isLoadingAllowedCountries = useState("isLoadingAllowedCountries", () => false);
  const countriesToShow = useState("countriesToShow", () => countries);
  const countryStatesDict = useState("countryStatesDict", () => ({}));
  const isLoadingCountryStates = useState("isLoadingCountryStates", () => ({}));
    ...
    try {
      ...
      if (response.allowedCountries) {
        ...
        countriesToShow.value = countries.filter(
          (country) => {
            var _a;
            return (_a = allowedCountries.value) == null ? void 0 : _a.includes(country.code);
          }
        );
      }
    } catch (error) {
  ...
  async function getStatesForCountry(countryCode) {
    ...
    try {
      ...
      if (countryStates) {
        countryStatesDict.value[countryCode] = countryStates;
      }
    } catch (error) {
...
const _sfc_main$1 = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      var _a;
      if ((_a = unref(countryStatesDict)[props.countryCode]) == null ? void 0 : _a.length) {
        _push(`<select${ssrRenderAttrs(mergeProps({ class: "h-[42px]" }, _attrs))}><option value=""${ssrIncludeBooleanAttr(!props.modelValue) ? " selected" : ""}>Select a state</option><!--[-->`);
        ssrRenderList(unref(countryStatesDict)[props.countryCode], (state) => {
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<select${ssrRenderAttrs(mergeProps({
        ...
        required: "",
        class: "h-[42px]"
      }, _attrs))}><option value="" disabled>Select a country</option><!--[-->`);
...
//# sourceMappingURL=CountrySelect-BEXsV39Z.mjs.map
